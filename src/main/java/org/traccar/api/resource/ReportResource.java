...
import org.traccar.iksupdate.MovingPosition;
...
public class ReportResource extends SimpleObjectResource<Report> {
    ...
    @Inject
    private MovingPosition movingPosition;
    ...
    @Path("route/moving/position")
    @GET
    public Collection<Position> getCurrentRoute(
            @QueryParam("deviceId") List<Long> deviceIds,
            @QueryParam("groupId") List<Long> groupIds) throws StorageException {
        permissionsService.checkRestriction(getUserId(), UserRestrictions::getDisableReports);
        return movingPosition.getObjects(getUserId(), deviceIds, groupIds);
    }
